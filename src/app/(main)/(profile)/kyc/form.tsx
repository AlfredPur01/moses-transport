...
export default function KycFormScreen() {
  ...
  const canAdvance = () => {
    switch (step) {
      ...
      case 6: return !!gName.trim() && !!gPhone.trim() && gConsent;
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (gPhotoB64   && userId) gPhotoUrl   = await upload(gPhotoB64,   `kyc/${userId}/guarantor`) ?? undefined;
        ...
        >
          ...
          {step === 0 && (
            <>
              ...
              <Field label="Date of Birth *" value={dob} onChange={setDob} placeholder="YYYY-MM-DD" keyboardType="numbers-and-punctuation" />
              <SelectRow<Gender>
                label="Gender *"
                value={gender}
                onChange={setGender}
            ...
            <>
              ...
              >
                ...
                <Text style={styles.consentTxt}>
                  The guarantor has given their informed consent to be listed as a guarantor for this autopay application. *
                </Text>
              </TouchableOpacity>
            </>
          )}
